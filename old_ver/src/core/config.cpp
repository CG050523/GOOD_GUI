...
void Config::init(prcopt_t* popt, ftpopt_t* fopt) {
    ...
    fopt->minus_add_1day = true;  /* (0:off  1:on) the day before and after the current day for precise satellite orbit
                                     and clock  products downloading */
    ...
    fopt->l2s4obs = 1; /* valid only for the observation files with long name, 0: long name, 1: short name, 2: long and
                          short name */
    fopt->getnav = false; /* (0:off  1:on) broadcast ephemeris */
    fopt->l2s4nav =
        1; /* valid only for the navigation files with long name, 0: long name, 1: short name, 2: long and short name */
    fopt->getorbclk = false; /* (0:off  1:on) precise orbit 'sp3' and precise clock 'clk' */
    fopt->l2s4oc = 1; /* valid only for the precise orbit and clock files with long name, 0: long name, 1: short name,
                         2: long and short name */
    ...
    fopt->l2s4snx = 1; /* valid only for IGS weekly SINEX files with long name, 0: long name, 1: short name, 2: long and
                          short name */
    ...
    fopt->l2s4ion =
        1; /* valid only for the ionosphere files with long name, 0: long name, 1: short name, 2: long and short name */
    fopt->getroti = false; /* (0:off  1:on) rate of TEC index (ROTI) */
    fopt->gettrp = false;  /* (0:off  1:on) CODE and/or IGS tropospheric product */
    fopt->l2s4trp = 1; /* valid only for the troposphere files with long name, 0: long name, 1: short name, 2: long and
                          short name */
    fopt->getatx = false; /* (0:off  1:on) ANTEX format antenna phase center correction */
} /* end of init */

void Config::init_GUI(prcopt_t* popt, ftpopt_t* fopt) {
    /* time settings */
    popt->ts = {0};  /* start time for processing */
    popt->ndays = 1; /* number of consecutive days */

    /* third party setting */
    popt->dir3party = "thirdparty";

    /* FTP downloading settings */
    fopt->minus_add_1day = true; /* (0:off  1:on) the day before and after the current day for precise
                                    satellite orbit and clock  products downloading */
    fopt->merge_sp3files = true; /* (0: off  1: on) to merge three consecutive sp3 files into one file */
    fopt->printinfo4wget = true; /* (0:off  1:on) print the information generated by 'wget' */

    /* initialization for FTP options */
    fopt->ftpdownloading = true; /* the master switch for data downloading, 0:off  1:on, only for data downloading */
    fopt->getobs = false;         /* (0:off  1:on) IGS observation (RINEX version 2.xx, short name 'd') */
    fopt->l2s4obs = 0; /* valid only for the observation files with long name, 0: long name, 1: short name, 2: long and
                          short name */
    fopt->getnav = false; /* (0:off  1:on) broadcast ephemeris */
    fopt->l2s4nav =
        0; /* valid only for the navigation files with long name, 0: long name, 1: short name, 2: long and short name */
    fopt->getorbclk = false; /* (0:off  1:on) precise orbit 'sp3' and precise clock 'clk' */
    fopt->l2s4oc = 0; /* valid only for the precise orbit and clock files with long name, 0: long name, 1: short name,
                         2: long and short name */
    fopt->geteop = false; /* (0:off  1:on) earth rotation parameter */
    fopt->l2s4eop = 0; /* valid only for EOP file with long name, 0: long name, 1: short name, 2: long and short name */
    fopt->getobx = false; /* (0:off  1:on) ORBEX (ORBit EXchange format) for satellite attitude information */
    fopt->getsnx = false; /* (0:off  1:on) IGS weekly SINEX */
    fopt->l2s4snx = 0; /* valid only for IGS weekly SINEX files with long name, 0: long name, 1: short name, 2: long and
                          short name */
    fopt->getdsb = false; /* (0:off  1:on) differential code/signal bias (DCB/DSB) */
    fopt->getosb = false; /* (0:off  1:on) observable-specific signal bias (OSB) */
    fopt->getion = false; /* (0:off  1:on) global ionosphere map (GIM) */
    fopt->l2s4ion =
        0; /* valid only for the ionosphere files with long name, 0: long name, 1: short name, 2: long and short name */
    fopt->getroti = false; /* (0:off  1:on) rate of TEC index (ROTI) */
    fopt->gettrp = false;  /* (0:off  1:on) CODE and/or IGS tropospheric product */
    fopt->l2s4trp = 0; /* valid only for the troposphere files with long name, 0: long name, 1: short name, 2: long and
                          short name */
    fopt->getatx = false; /* (0:off  1:on) ANTEX format antenna phase center correction */
}
...
bool Config::ReadCfgYaml(std::string cfgfile, prcopt_t* popt, ftpopt_t* fopt) {
    ...
    try {
        ...
        Logger::Trace(TERROR, "*** ERROR(Config::ReadCfgYaml): Failed to read the configuration file. Please check the "
                              "path and format of the configuration file!");
    ...
    } catch (YAML::Exception& exception) { msg.push_back("eopDir"); }
    ...
    } catch (YAML::Exception& exception) { msg.push_back("obxDir"); }
    ...
    } catch (YAML::Exception& exception) { msg.push_back("3partyDir"); }
    ...
    try {
        ...
        if (j == 1) /* year, month, day */
        {
            if (itemp.size() != 5) {
                Logger::Trace(
                    TERROR,
                    "*** ERROR(PreProcess::ReadCfgYaml): the setting of 'year, month, day' is wrong, please check it!");
                return false;
            }
            std::vector<double> ep;
            ep.push_back(itemp[1]);
            ep.push_back(itemp[2]);
            ep.push_back(itemp[3]);
            ep.push_back(0);
            ep.push_back(0);
            ep.push_back(0);
        ...
        {
            if (itemp.size() != 4) {
                Logger::Trace(TERROR, "*** ERROR(PreProcess::ReadCfgFile): the setting of 'year, day of year' is "
        ...
        {
            if (itemp.size() != 4) {
                Logger::Trace(TERROR, "*** ERROR(PreProcess::ReadCfgFile): the setting of 'GPS week, day within week' "
    ...
    } catch (YAML::Exception& exception) { msg.push_back("procTime"); }

    /* FTP downloading settings */
    try {
        /* (0:off  1:on) the day before and after the current day for precise satellite orbit and clock products
         * downloading */
        fopt->minus_add_1day = config["minusAdd1day"].as<int>() == 1 ? true : false;
    } catch (YAML::Exception& exception) { msg.push_back("minusAdd1day"); }
    ...
    } catch (YAML::Exception& exception) { msg.push_back("merge_sp3files"); }

    try {
        /* (0:off  1:on) print the information generated by 'wget' */
        fopt->printinfo4wget = config["printInfoWget"].as<int>() == 1 ? true : false;
    } catch (YAML::Exception& exception) { msg.push_back("printInfoWget"); }
    ...
    } catch (YAML::Exception& exception) { msg.push_back("ftpDownloading"); }

    int hh = 0, nh = 0, imax = 0, step = 1;
    try {
        fopt->getobs =
            config["getObs"]["opt4obs"].as<int>() == 1 ? true : false; /* (0:off  1:on) GNSS observation data */
        ...
        for (int i = hh; i < imax; i++)
            fopt->hhobs.push_back(i);
        fopt->l2s4obs = config["getObs"]["l2s4obs"].as<int>();
    } catch (YAML::Exception& exception) { msg.push_back("getObs"); }

    try {
        fopt->getnav =
            config["getNav"]["opt4nav"].as<int>() == 1 ? true : false; /* (0:off  1:on) broadcast ephemeris */
        ...
        imax = MIN(hh + nh, 24);
        for (int i = hh; i < imax; i++)
            fopt->hhnav.push_back(i);
        fopt->l2s4nav = config["getNav"]["l2s4nav"].as<int>();
    } catch (YAML::Exception& exception) { msg.push_back("getNav"); }

    try {
        fopt->getorbclk =
            config["getOrbClk"]["opt4oc"].as<int>() == 1 ? true : false; /* (0:off  1:on) precise orbit and clock */
        ...
        for (size_t i = 0; i < acs.size(); i++) {
            ...
            for (int i = hh; i < imax; i += step) {
                if (ac_i == "esa_u")
                    fopt->hhorbclk[0].push_back(i);
                else if (ac_i == "gfz_u")
                    fopt->hhorbclk[1].push_back(i);
                else if (ac_i == "igs_u")
                    fopt->hhorbclk[2].push_back(i);
                else if (ac_i == "whu_u")
                    fopt->hhorbclk[3].push_back(i);
            }
        }

        fopt->l2s4oc = config["getOrbClk"]["l2s4oc"].as<int>();
    } catch (YAML::Exception& exception) { msg.push_back("getOrbClk"); }

    try {
        fopt->geteop =
            config["getEop"]["opt4eop"].as<int>() == 1 ? true : false; /* (0:off  1:on) earth rotation parameter */
        ...
        imax = MIN(hh + nh * step, 24);
        for (int i = hh; i < imax; i += step)
            fopt->hheop.push_back(i);

        fopt->l2s4eop = config["getEop"]["l2s4eop"].as<int>();
    } catch (YAML::Exception& exception) { msg.push_back("getEop"); }

    try {
        fopt->getobx = config["getObx"]["opt4obx"].as<int>() == 1 ?
                           true :
                           false; /* (0:off  1:on) ORBEX (ORBit EXchange format) for satllite attitude information */
        fopt->obxfrom = config["getObx"]["obxFrom"].as<std::string>();
    } catch (YAML::Exception& exception) { msg.push_back("getObx"); }

    try {
        fopt->getdsb = config["getDsb"]["opt4dsb"].as<int>() == 1 ?
                           true :
                           false; /* (0:off  1:on) differential code/signal bias (DCB/DSB) */
        fopt->dsbfrom = config["getDsb"]["dsbFrom"].as<std::string>();
    } catch (YAML::Exception& exception) { msg.push_back("getDsb"); }

    try {
        fopt->getosb = config["getOsb"]["opt4osb"].as<int>() == 1 ?
                           true :
                           false; /* (0:off  1:on) observable-specific signal bias (OSB) */
        fopt->osbfrom = config["getOsb"]["osbFrom"].as<std::string>();
    } catch (YAML::Exception& exception) { msg.push_back("getOsb"); }
    ...
    } catch (YAML::Exception& exception) { msg.push_back("getSnx"); }

    try {
        fopt->getion = config["getIon"]["opt4ion"].as<int>() == 1 ?
                           true :
                           false; /* (0:off  1:on) CODE and/or IGS global ionosphere map (GIM) */
        fopt->ionfrom = config["getIon"]["ionFrom"].as<std::string>();
        fopt->l2s4ion = config["getIon"]["l2s4ion"].as<int>();
    } catch (YAML::Exception& exception) { msg.push_back("getIon"); }

    try {
        fopt->getroti =
            config["getRoti"]["opt4rot"].as<int>() == 1 ? true : false; /* (0:off  1:on) Rate of TEC index (ROTI) */
    } catch (YAML::Exception& exception) { msg.push_back("getRoti"); }

    try {
        fopt->gettrp = config["getTrp"]["opt4trp"].as<int>() == 1 ?
                           true :
                           false; /* (0:off  1:on) CODE and/or IGS tropospheric product */
    ...
    } catch (YAML::Exception& exception) { msg.push_back("getTrp"); }

    try {
        fopt->getatx = config["getAtx"]["opt4atx"].as<int>() == 1 ?
                           true :
                           false; /* (0:off  1:on) ANTEX format antenna phase center correction */
    } catch (YAML::Exception& exception) { msg.push_back("getAtx"); }

    if ((int)msg.size() > 0) {
        Logger::Trace(TERROR, "*** ERROR(PreProcess::ReadCfgYaml): Failed when loading the following settings: ");
        for (size_t i = 0; i < msg.size(); i++)
            Logger::Trace(TERROR, "  " + std::to_string(i + 1) + ". " + msg[i]);
